...
void InstructionDecoder::registerInstructions(CPU* cpu) {
    ...
    handlers = {
        ...
        // op_ld_r_r handles all LD r,r' including (HL) variants with proper cycle count
        {Op(0xC7, 0x06), &CPU::op_ld_r_imm},
        {Op(0xF8, 0x70), &CPU::op_ld_hl_ind_r},  // LD (HL),r - writes to memory
        {Op(0xFF, 0x36), &CPU::op_ld_hl_ind_imm}, // LD (HL),n - writes to memory
        {Op(0xC0, 0x40), &CPU::op_ld_r_r},        // LD r,r' including LD r,(HL)
        ...
        // 8-bit arithmetic/logic - generic _r handlers include (HL) with proper cycle count
        {Op(0xFF, 0xC6), &CPU::op_add_imm},
        {Op(0xF8, 0x80), &CPU::op_add_r},  // includes ADD A,(HL)
        {Op(0xFF, 0xCE), &CPU::op_adc_imm},
        {Op(0xF8, 0x88), &CPU::op_adc_r},  // includes ADC A,(HL)
        {Op(0xFF, 0xD6), &CPU::op_sub_imm},
        {Op(0xF8, 0x90), &CPU::op_sub_r},  // includes SUB (HL)
        {Op(0xFF, 0xDE), &CPU::op_sbc_imm},
        {Op(0xF8, 0x98), &CPU::op_sbc_r},  // includes SBC A,(HL)
        {Op(0xFF, 0xFE), &CPU::op_cp_imm},
        {Op(0xF8, 0xB8), &CPU::op_cp_r},   // includes CP (HL)
        {Op(0xC7, 0x04), &CPU::op_inc_r},  // includes INC (HL)
        {Op(0xC7, 0x05), &CPU::op_dec_r},  // includes DEC (HL)
        {Op(0xFF, 0xE6), &CPU::op_and_imm},
        {Op(0xF8, 0xA0), &CPU::op_and_r},  // includes AND (HL)
        {Op(0xFF, 0xF6), &CPU::op_or_imm},
        {Op(0xF8, 0xB0), &CPU::op_or_r},   // includes OR (HL)
        {Op(0xFF, 0xEE), &CPU::op_xor_imm},
        {Op(0xF8, 0xA8), &CPU::op_xor_r},  // includes XOR (HL)
